...
# %% auto 0
__all__ = ['force', 'df', 'read', 'convert_bq_dtypes', 'to', 'get_size_gb', 'ex', 'exists', 'Table', 'build']
...
# %% ../nbs/00_core.ipynb 8
def exists(table_id, verbose=False):
    client = bigquery.Client()
    try:
        client.get_table(table_id)  
        if verbose: print(f"Table {table_id} already exists")
        return True
    except NotFound:
        if verbose: print(f"Table {table_id} is not found.")
        return False
exists("petsathome-sb-datascience.temporary.own_brand_product_intervals")
...
def build(table: Table, query, force=False, read=True):
    if not exists(table.id, verbose=True) or force:
        pbq.ex(query)
    if read:
        df = pbq.read(table.id)
        return df
force = False
df = build(table, get_query(table), force=force)
